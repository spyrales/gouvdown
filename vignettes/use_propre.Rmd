...
- au format long : **LIEN DOCUMENTATION LONGUE PROPRE** ;
- au format court : **LIEN GITHUB PROPRE.BROCHURE** (cette brochure est-elle même réalisée grâce à la fonction `propre_brochure` présentée ici).
...
Il faut retenir que l'indentation est très important car elle indique de quel élément va dépendre chaque ligne. Lorsque le fichier bloque dès le début de la compilation, c'est l'une des premières erreurs à rechercher.
...
- le format de sortie (`output`) et des arguments qui le précise (`logo`, `width_main_column`). C'est ce qui permet d'utiliser la fonction `propre_brochure` ;
- des arguments complémentaires (`knit`), qui sert ici à générer un fichier PDF directement en imprimant le contenu du fichier HTML.

### Balisage

RMarkdown repose sur Markdown qui, contrairement à d'autres outils comme Work ou LibreOffice, n'affiche pas directement la mise en forme du document. Il faut une étape de compilation (en appuyant sur le boutant _Knit_ ou en utilisant la fonction `rmarkdown::render`) pour cela.

Cela implique que la mise en forme est directement incluse dans le texte au moyen de balises qui permettent d'indiquer le niveau du titre, si il s'agit d'un bloc de citation, d'un texte en gras, en italique ou d'une note de bas de page.
...
Soit en utilisant la syntaxe HTML :
...
- `logo` : bloque-marque à afficher. La liste des bloques-marques disponibles est présente [ici](https://github.com/spyrales/gouvdown/tree/master/inst/resources/blocs_marque). Si le votre n'y est pas, n'hésitez pas à en proposer l'ajout.
- `width_main_column` : largeur (en cm) de la colonne de texte principale (de la classe `main-column`).

CAPTURE ECRAN PROPRE AVEC LOGO ET YAML EN FACE - 2 situations

IDEM WIDTH MAIN COLUMN - 2 situations
...
METTRE CODE ET RESULTAT EN FACE A FACE SUR TITRES / GRAS / ITALIQUE / TEXTE / BLOCKQUOTE
...
```{r graph_gouvdown, fig.width=8, fig.height=4}
library(gouvdown)
library(ggplot2)
...
SECTION A ECRIRE APRES LE PASSAGE SUR LE CSS

gestion de la pagination
utlisation des divs de colonnes

autres divs présents dans les ressources
...
Pour cela on l'ouvre avec Chrome ou Chromium et on utlise l'option _Inspecter_ (faire clic-droit sur un élément).

CAPTURE D'ECRAN INSPECTER BOUTON
...
Il est possible de créer des classes personnalisées dans le fichier. On peut alors leur donner des valeurs et les utliser pour pousser davantage la personnalisation.
